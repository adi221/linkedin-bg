// product actions
export const ADD_ICON_TO_PRODUCT = 'ADD_ICON_TO_PRODUCT';
export const UPDATE_BG_COLOR = 'UPDATE_BG_COLOR';
export const SET_ICONS = 'SET_ICONS';
export const LIST_ICONS = 'LIST_ICONS';

// alert actions
export const SET_ALERT = 'SET_ALERT';

// upload actions
export const SHOW_UPLOAD_MODAL = 'SHOW_UPLOAD_MODAL';
export const ADD_ICON_TO_LIST_REQUEST = 'ADD_ICON_TO_LIST_REQUEST';
export const ADD_ICON_TO_LIST_SUCCESS = 'ADD_ICON_TO_LIST_SUCCESS';
export const ADD_ICON_TO_LIST_FAIL = 'ADD_ICON_TO_LIST_FAIL';
